...
if __name__ == "__main__":
    ...
    PPAS = np.array([25.0,40.0,50.0,60.0,70.0,80.0,90.0,100.0])
    
    for j in range(len(PPAS)):
        for i in range(nruns):
            ...
            if save:
                file = open('final_results3/variable_profit/%s_profit.txt'%(ppa), 'a')
                file.write('%s'%(opt_val) + '\n')
                file.close()

                file = open('final_results3/variable_profit/%s_x.txt'%(ppa), 'a')
                file.write('%s'%(repr(xf)) + '\n')
                file.close()

                file = open('final_results3/variable_profit/%s_y.txt'%(ppa), 'a')
                file.write('%s'%(repr(yf)) + '\n')
                file.close()

                file = open('final_results3/variable_profit/%s_time.txt'%(ppa), 'a')
                file.write('%s'%(run_time) + '\n')
                file.close()

                file = open('final_results3/variable_profit/%s_calls.txt'%(ppa), 'a')
                file.write('%s'%(function_calls) + '\n')
                file.close()

                file = open('final_results3/variable_profit/%s_history.txt'%(ppa), 'a')
